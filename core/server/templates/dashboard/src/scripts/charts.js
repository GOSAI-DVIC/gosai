...
function update_ms_chart(new_entries) {
    ...
    for (const [name, ms_time] of Object.entries(new_entries.data)) {
        if (
            data.datasets.find((dataset) => dataset.label == name) == undefined
        ) {
            let len;
            ...
            data.datasets.push({
                ...
                borderColor: ms_colors[data.datasets.length % ms_colors.length],
                backgroundColor: ms_colors[data.datasets.length % ms_colors.length],
            });
...
const drivers_lp_ctx = document
    .getElementById("drivers-looptime-chart")
    .getContext("2d");
...
function update_drivers_lp_chart(new_entries) {
    ...
    for (const dataset of data.datasets) {
        ...
        if (driver_data != undefined && driver_data["loop_time"] != undefined)
    ...
    for (const [name, driver_data] of Object.entries(new_entries.data)) {
        if (
            driver_data["loop_time"] != undefined &&
            data.datasets.find((dataset) => dataset.label == name) == undefined
        ) {
            let len;
            if (data.datasets.length == 0) len = 0;
            ...
            data.datasets.push({
                ...
                data: new Array(len).fill(0),
                borderColor:
                    ms_colors[(ms_colors.length - 1 - data.datasets.length) % ms_colors.length],
                backgroundColor:
                    ms_colors[(ms_colors.length - 1 - data.datasets.length) % ms_colors.length],
            });
            let dataset = data.datasets[data.datasets.length - 1];
...
const drivers_nlp_data = {
    datasets: [],
};

const drivers_nlp_config = {
    type: "line",
    data: drivers_nlp_data,
    options: {
        color: "#ffffff",
        responsive: true,
        maintainAspectRatio: false,
...
const drivers_nlp_ctx = document
    .getElementById("drivers-not-looptime-chart")
    .getContext("2d");
const drivers_nlp_chart = new Chart(drivers_nlp_ctx, drivers_nlp_config);

function update_drivers_nlp_chart(new_entries) {
    let data = drivers_nlp_chart.data;
    data.labels.push(new Date(new_entries.timestamp).getSeconds());
    for (const dataset of data.datasets) {
        let [driver_name, data_name] = dataset.label.split("_-_")
        // console.log(driver_name, data_name)
        let driver_data = new_entries.data[driver_name];
        if (driver_data != undefined && driver_data[data_name] != undefined)
            dataset.data.push(driver_data[data_name]);
        else dataset.data.push(0);
    }

    for (const [name, driver_data] of Object.entries(new_entries.data)) {
        for (const [data_name, data_value] of Object.entries(driver_data)) {
            if (
                data_name != "loop_time" &&
                data.datasets.find((dataset) => dataset.label == name + "_-_" + data_name) == undefined
            ) {
                let len;
                if (data.datasets.length == 0) len = 0;
                else len = data.datasets[0].data.length - 1;
                data.datasets.push({
                    label: name + "_-_" + data_name,
                    data: new Array(len).fill(0),
                    borderColor:
                        ms_colors[(4 + data.datasets.length) % ms_colors.length],
                    backgroundColor:
                        ms_colors[(4 + data.datasets.length) % ms_colors.length],
                });
                let dataset = data.datasets[data.datasets.length - 1];
                dataset.data.push(data_value);
            }
        }
    }
    drivers_nlp_chart.update();
    for (const dataset of data.datasets) {
        if (dataset.data.length > 100) {
            dataset.data.shift();
        }
...
function objetContainsOtherThat(obj, other) {
